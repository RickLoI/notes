...
\begin{document}
    Projective spaces are deeply rooted in art,
        as these were implicitly when dealing with perspective.
        As it turns out,
    ...
    Points in \(\mathbb{P}^{n}(\K)\) are represented using two types of coordinates:
        \begin{enumerate}
            ...
            \item \emph{non-homogeneous} coordinates, by which \(P\) is written as 
                ...
                \end{cases}\]
        \end{enumerate}
        In the latter case, we say that \(P\) is proper (or affine) point, if \(x_{0} \ne 0\),
        we say that it's an improper point (or a point at infinity) if \(x_{0} = 0\).

    Our discussion on elliptic curves will involve points in \(\mathbb{P}^{2}(\K)\),
        and we mainly use the non-homogeneous coordinates system.
